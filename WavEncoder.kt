...
class WaveEncoder(sampleRate: Int, samples: ShortArray) {
    val sampleRate = sampleRate
    val samples = samples

    fun makeWavHeader(): ByteArray {
        // fill little indians
    ...
    fun getData(): ByteArray {
        ...
        var data = ByteBuffer.allocate(samples.size * 2).order(ByteOrder.LITTLE_ENDIAN)
        for(s in samples) {
            data.putShort(s)
        }

        var result = ByteArray(44 + samples.size * 2)
